...
export default function BlogCard({ heading, date, description, link }: BlogCardProps) {
    ...
    return (
        ...
        <a href={link} target="_blank" rel="noopener noreferrer" className="w-full">
            <div className="flex flex-col justify-start items-start gap-2 p-4 bg-neutral-100/40  dark:bg-neutral-950  rounded-md w-full z-10">
                <h1 className="text-xl font-medium z-50">{heading}</h1>
                <div className="flex flex-row justify-between gap-8 w-full text-sm z-50">
                    <a className="z-50">{description}</a>
                </div>
                <div className="flex justify-between gap-8 w-full items-center gap-2 z-50">
